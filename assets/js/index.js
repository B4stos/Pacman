...
function preload ()
{
    ...
    this.load.image("pill", "assets/images/pac man pill/pill0.png");
    this.load.image("lifecounter", "assets/images/pac man life counter/contadorvidas.png");
}
...
{    
    ...
    this.anims.create({
        ...
        this.input.keyboard.on('keydown', function(event) {
            lastKeys.push(event.key.toUpperCase());
            if (lastKeys.length > 10) {
                lastKeys.shift();
            }
            if (lastKeys.join(',') === cheatCode.join(',')) {
                numLives = 100;
            }
        });
        var livesText = 'Lives: ' + numLives;
        this.add.text(10, 10, livesText, { font: '24px Arial', fill: '#ffffff' });
    }
    ...
    let skins=[Animation.Ghost.Blue, Animation.Ghost.Red, Animation.Ghost.Orange , Animation.Ghost.Pink];
     map.filterObjects("Objects", function (value, index, array) {        
        if(value.name == "Ghost") {
            let posicao = new Phaser.Geom.Point(value.x + offset, value.y - offset);
            let ghost = new Ghost(scene, posicao, skins[i]);
            ghosts.push(ghost);    
